---
title: 'Rustzen Minimalist Architecture Design Guide: From Chaos to Clarity in Backend Evolution Practice'
...
- In AI collaboration, due to unclear code structure, **generated content was redundant and difficult to combine and reuse**.
...
## IV. Authentication & Authorization: JWT-based System

**System Overview:**

- User authentication and authorization are both handled via JWT tokens.
- After user login, the system issues a JWT that contains user identity and permission claims.
- This stateless design simplifies deployment and scaling.
- Permission caching improves performance and reduces database load.

### 4.1 Middleware Architecture

#### Core Middleware Components

- ✅ **JWT Authentication Middleware**: Verifies the token, validates user identity, and extracts claims.
- ✅ **Permission Verification Middleware**: Checks user permissions based on the claims in the JWT.

#### Middleware Implementation Example

```rust
// JWT authentication middleware - handles both auth and authorization
pub async fn jwt_auth_middleware(
    mut request: Request,
    next: Next,
) -> Result<Response, AppError> {
    let token = extract_token_from_header(&request)?;
    // JWT contains both user identity and permission claims
...
### 4.2 Permission Caching and Renewal Mechanism

Why design it this way?

> Permission caching reduces database pressure, renewal mechanism ensures active user experience, automatic expiration cleanup enhances security.
...
- ✅ **Permission Renewal Mechanism**: Automatically renews 1 hour each time user info interface is accessed
- ✅ **User Status Changes**: Cache immediately cleared when user is disabled or kicked offline
- ⚙️ **Future Extensions**: Add refresh token mechanism for better security
...
- **Permission Granularity**: Current project's permission granularity is sufficiently fine-grained
- **Permission Codes**: Such as `system:user:list`, `system:user:create`, etc.
- **Wildcard Support**: `system:*` supports module general permissions

**Note**: The current project doesn't need more fine-grained permission control, current granularity meets business requirements.
    ...
    fn from(err: ServiceError) -> Self {
        let (status, code, message) = match err {
            ...
            ServiceError::InvalidToken => (
                StatusCode::FORBIDDEN,  // 改为 FORBIDDEN，因为这是授权问题
                30000,
...
Rustzen is not built for AI, but its clarity — designed for human understanding — happens to align well with how AI assistants interpret and generate code.
...
Minimalism is not "doing less", but **precise design, just right**; it provides a clearer foundation for AI collaboration, team cooperation, and future expansion.
...
- 🔄 **Authentication Flow**: Implement proper session-based authentication with secure session management
- 🔄 **JWT Optimization**: Use JWT only for authorization, implement refresh token mechanism
...
- 🔄 **Documentation Improvement**: More comprehensive API documentation, facilitating AI analysis
- 🔄 **Log Optimization**: Optimize log format, facilitating AI analysis
